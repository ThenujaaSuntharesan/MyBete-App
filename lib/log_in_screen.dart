...
import 'package:mybete_app/sign_up_screen.dart'; // Create a HomePage to navigate after login
...
class _LoginPageState extends State<LoginPage> {
  ...
  void login() async {
    try {
      var userDoc = await _firestore.collection("users").doc(usernameController.text.trim()).get();
      if (userDoc.exists) {
        ...
        Navigator.pushReplacement(
            context, MaterialPageRoute(builder: (context) => DiabeteOptions())
        );
      } else {
